...
function Game:set_globals()
    ...
    -- Web (love.js / WebAssembly) — no LuaJIT, no threads, limited OS API
    if love.system.getOS() == 'Web' then
        self.F_SOUND_THREAD    = false  -- audio on main thread; thread API unavailable
        self.F_HTTP_SCORES     = false  -- no https library in web
        self.F_CRASH_REPORTS   = false  -- crash reporter uses threads + https
        self.F_EXTERNAL_LINKS  = false  -- openURL unreliable without user gesture
        self.F_LOCAL_CLIPBOARD = true   -- browser clipboard requires async permissions
        self.F_ENGLISH_ONLY    = false
        self.F_VIDEO_SETTINGS  = false  -- no desktop display / fullscreen modes
        self.F_CTA             = false  -- newVideo not supported in love.js

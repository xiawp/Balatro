...
function GET_DISPLAYINFO(screenmode, display)
  -- Web has no desktop display/fullscreen concept — return minimal stub
  if love.system.getOS() == 'Web' then
    local w, h = love.graphics.getWidth(), love.graphics.getHeight()
    G.SETTINGS.WINDOW.display_names = {'1'}
    G.SETTINGS.WINDOW.DISPLAYS = G.SETTINGS.WINDOW.DISPLAYS or {}
    G.SETTINGS.WINDOW.DISPLAYS[1] = {
      DPI_scale = 1,
      MONITOR_DIMS = {width = w, height = h},
      screen_resolutions = { strings = {w..' X '..h}, values = {{w=w, h=h}} },
    }
    return 1
  end

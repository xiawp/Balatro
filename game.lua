...
function Game:start_up()
    ...
    G.SAVE_MANAGER = {
        thread = love.system.getOS() ~= 'Web' and love.thread.newThread('engine/save_manager.lua') or nil,
        channel = love.thread.getChannel('save_request')
    }
    if G.SAVE_MANAGER.thread then G.SAVE_MANAGER.thread:start(2) end
    boot_timer('savemanager', 'shaders',0.4)

    --call the http manager
    G.HTTP_MANAGER = {
        thread = love.system.getOS() ~= 'Web' and love.thread.newThread('engine/http_manager.lua') or nil,
        out_channel = love.thread.getChannel('http_request'),
        in_channel = love.thread.getChannel('http_response')
    }
    if G.F_HTTP_SCORES and G.HTTP_MANAGER.thread then
